...
class Search extends CI_Controller {
	...
	public $db = 'pubmed';
	public $retmode = 'xml';
	public $exact_match = false; // Exact match narrows the search results by wrapping in quotes
	...
	function dosearch()
	{
        ...
        $this->form_validation->set_rules('search_term', 'search_term', 'trim|required');
        $term = $this->input->post('search_term');
        $searchResults = $this->query($term);
	...
	{
		// need to enable Curl in apache - apt-get install php5-curl and then /etc/init.d/apache2 restart
		$curl = curl_init();
